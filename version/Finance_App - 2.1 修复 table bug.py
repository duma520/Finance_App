...
class FinanceApp(QMainWindow):
    ...
    def apply_filters(self):
        ...
        if search_text:
            ...
            # 2. 匹配金额
            query += " OR CAST(amount AS TEXT) LIKE ?"
            params.append(f"%{search_text}%")
            ...
            if pinyin_descriptions:
                query += "description IN (" + ",".join(["?"]*len(pinyin_descriptions)) + ")"
                params.extend(pinyin_descriptions)
                
                if pinyin_categories:
                    query += " OR "
            
            if pinyin_categories:
                query += "category IN (" + ",".join(["?"]*len(pinyin_categories)) + ")"
                params.extend(pinyin_categories)
