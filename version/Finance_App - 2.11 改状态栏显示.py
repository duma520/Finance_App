...
class FinanceApp(QMainWindow):
        ...
        if login_dialog.exec_() == QDialog.Accepted:
            ...
            if new_user:
                ...
                try:
                    ...
                    self.statusBar().showMessage("❌ 用户名已存在!", 3000)  # 显示3秒
    ...
    def add_record(self):
        ...
        if not category:
            self.statusBar().showMessage("❌ 请选择类别!", 3000)
        ...
        if type_text == "余额":
            ...
            self.statusBar().showMessage(f"✅ 余额记录已添加: {amount:.2f} 元", 3000)
        ...
        # 对于非余额记录，验证金额输入
        if not amount_text:
            self.statusBar().showMessage("⚠️ 请输入金额!", 3000)
        ...
        try:
            ...
            if amount <= 0:
                self.statusBar().showMessage("❌ 金额必须大于0!", 3000)
                self.amount_edit.selectAll()
                self.amount_edit.setFocus()
                return
        except ValueError:
            self.statusBar().showMessage("❌ 金额必须是有效数字!", 3000)
            self.amount_edit.selectAll()
            self.amount_edit.setFocus()
            return
        ...
        if self.date_edit.date() > QDate.currentDate():
            self.statusBar().showMessage("❌ 日期不能晚于今天!", 3000)
            self.date_edit.setFocus()
            return
        
        # 描述长度限制
        if len(description) > 100:
            self.statusBar().showMessage("❌ 描述不能超过100个字符!", 3000)
        ...
        if type_text == "借款":
            ...
            self.statusBar().showMessage("✅ 借款记录已添加!", 3000)
        
        elif type_text == "还款":
            # 添加还款记录并关联借款
            loan_id = self.loan_combo.currentData()
            if not loan_id:
                self.statusBar().showMessage("❌ 没有可关联的借款!", 3000)
                return
            ...
            if total_repaid >= loan_amount:
                ...
                ''', (loan_id,))
                self.statusBar().showMessage("✅ 借款已全部还清!", 3000)
            else:
                self.statusBar().showMessage("✅ 还款记录已添加!", 3000)
            ...
            self.statusBar().showMessage(f"✅ {type_text}记录已添加!", 3000)
    ...
    def delete_record(self):
        ...
        if not selected_rows:
            self.statusBar().showMessage("❌ 请选择要删除的记录!", 3000)
            return
    ...
    def settle_record(self):
        ...
        if not selected:
            self.statusBar().showMessage("❌ 请选择要结算的记录!", 3000)
            return
        ...
        if record_type != '借款':
            self.statusBar().showMessage("❌ 只能结算借款记录!", 3000)
            return
        ...
        self.statusBar().showMessage("✅ 借款记录已标记为已结清!", 3000)
        self.load_data()
        self.update_loan_combo()
        self.update_statistics()
    ...
    def set_auto_backup(self):
        ...
        if ok and interval:
            ...
            self.statusBar().showMessage(f"✅ 已设置为{interval}自动备份", 3000)

    def restore_data(self):
        """恢复数据库"""
        backup_dir = "backups"
        if not os.path.exists(backup_dir):
            self.statusBar().showMessage("⚠️ 备份目录不存在!", 3000)
            return
    ...
    def undo_last_operation(self):
        ...
        try:
            ...
            self.load_data()
            self.update_statistics()
            self.statusBar().showMessage("✅ 已撤销最后一次操作!", 3000)
        ...
        if ok and name:
            ...
            if ok and type_:
                ...
                try:
                    ...
                    self.statusBar().showMessage("⚠️ 分类已存在!", 3000)

    def delete_category(self, dialog):
        """删除分类"""
        selected = self.category_list.currentItem()
        if not selected:
            self.statusBar().showMessage("❌ 请选择要删除的分类!", 3000)
            return
