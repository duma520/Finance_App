...
class FinanceApp(QMainWindow):
    ...
    def add_record_v1(self):
        ...
        if type_text == "账户结余":
            ...
            self.cursor.execute('''
                ...
                (type, amount, category, description, date, account_id)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', ('balance', amount, "账户结余统计", f"截至 {date} 的账户 {self.account_combo.currentText()} 结余", date, account_id))
            self.conn.commit()
        ...
        if type_text == "借款":
            ...
            ''', ('还款', amount, category, description, date, loan_id, account_id, tags,
                getattr(self, 'receipt_image_data', None), 0, None, None))  # 还款记录默认不是定期交易
        ...
        self.update_account_balances()  # 新增：更新UI中的账户余额显示
        self.load_data()
        self.update_loan_combo()
        self.update_statistics()
    ...
    def edit_record(self):
        ...
        self.load_account_combo_to(account_combo)  # 新方法，用于加载账户到指定下拉框
        ...
        update_form(type_combo.currentText())  # 初始化
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            # 更新后续日期的余额记录
            if record[0] in ["income", "expense", "借款", "还款"]:
                self.update_future_balances(date)
    ...
    def show_add_record_dialog(self):
        ...
        form_layout.addRow(receipt_btn.parent())  # 调整位置
        
        form_layout.addRow("频率:", recurring_freq_combo)
        form_layout.addRow("结束于:", recurring_end_edit)
    ...
    def add_record_from_dialog(self, type_text, amount_text, category, account_id, date, 
        ...
        """从弹窗添加记录的核心逻辑"""
        # 这里放入原来add_record方法的核心代码，从类型检查开始到最后的提交逻辑
        # 只需要复制原add_record方法的内容，去掉UI相关的部分
        ...
        if type_text == "借款":
            ...
            self.cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', ('还款', amount, category, description, date, loan_id, account_id, tags,
                receipt_image_data, 0, None, None))  # 还款记录默认不是定期交易
        ...
        self.update_account_balances()  # 更新UI中的账户余额显示
        self.load_data()
        self.update_loan_combo()
        self.update_statistics()
