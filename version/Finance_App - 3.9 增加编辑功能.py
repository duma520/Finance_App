...
class FinanceApp(QMainWindow):
    ...
    def create_balance_tab(self):
        ...
        refresh_btn.clicked.connect(self.update_account_balances)
    ...
    def update_account_balances(self):
        ...
        for row, (account_id, account_name) in enumerate(accounts):
            # 获取当前余额
            ...
            self.cursor.execute("""
                SELECT SUM(amount) FROM transactions 
                WHERE account_id=? AND (type='income' OR type='还款')
            """, (account_id,))
            ...
            self.cursor.execute("""
                SELECT SUM(amount) FROM transactions 
                WHERE account_id=? AND (type='expense' OR type='借款')
            """, (account_id,))
            total_expense = self.cursor.fetchone()[0] or 0
            
            # 设置表格项
            self.balance_table.setItem(row, 0, QTableWidgetItem(account_name))
