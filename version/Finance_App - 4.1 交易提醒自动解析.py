...
from PyQt5.QtCore import Qt, QDate, QTimer, QSize, QByteArray
...
def notice_parse_merchant(text):
    ...
    for label in NOTICE_MERCHANT_LABELS:
        for lm in re.finditer(re.escape(label), text):
...
def parse_transaction_notice(text, now=None):
    """解析交易提醒文本 → dict(amount, merchant, datetime, date, type)"""
    text = (text or "").strip()
    if not text:
        return {"amount": None, "merchant": None, "datetime": None, "date": None, "type": None}
...
class FinanceApp(QMainWindow):
    ...
    def load_settings(self):
        ...
        try:
            ...
            result = self.cursor.fetchone()
            if result and result[0]:
                self.restoreGeometry(QByteArray.fromBase64(result[0].encode('utf-8')))
    ...
    def show_add_record_dialog(self):
        ...
        notice_edit.setPlaceholderText(
            ...
            "粘贴后会自动提取【交易金额 / 交易商户 / 交易时间】并填写到下方字段。"
        )
